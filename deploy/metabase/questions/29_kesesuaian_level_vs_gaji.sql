...
JOIN dim_kesesuaian_level ON fact_tracer_study.kesesuaian_level_sk = dim_kesesuaian_level.kesesuaian_level_sk
...
WHERE fact_tracer_study.take_home_pay IS NOT NULL
  AND dim_kesesuaian_level.label IS DISTINCT FROM 'Tidak Ada Data'
  [[AND dim_alumni.tahun_lulus = {{tahun_lulus}}]]
  [[AND dim_prodi.nama_prodi = {{prodi}}]]
GROUP BY dim_kesesuaian_level.label
